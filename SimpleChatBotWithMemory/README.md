...
*   **	emperature=0.2**: We deliberately use a low temperature setting. This makes the model's responses more deterministic and logical, which is crucial for maintaining consistent context in a conversational memory chain without hallucinating.
...
*   **	emperature=0.2**: Hafızaya dayalı sürekli bir sohbette modelin mantıksal çerçeveden kopmaması ve halüsinasyon görmemesi (saçmalamaması) için sıcaklık oranı bilerek düşük tutulmuştur.
